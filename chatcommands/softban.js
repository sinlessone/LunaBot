...
module.exports = {
    async chatSoftBan(message, client) {
        ...
        if (command.length === 1) {
            return message.reply({
                embeds: [presets.warning("", "please provide a user to softban as such:\n?softban (user) (reason)")]
            })
        }
        ...
        if (/\D/.test(target)) {
            return message.reply({
                embeds: [presets.warning("", "please only either ping someone to softban or use their ID")]
            })
        }
        ...
        if (toBan) {
            ...
            if (message.member.id === toBan.id) {
                return message.reply({
                    embeds: [presets.warning("", `You cannot softban your self, silly!`)],
                })
            }
